...
//
// FILES (#370). Every path in an entry's `files` must exist in the tree, and an
...
lines.forEach((line, i) => {
  ...
  if ('files' in d && !(Array.isArray(d.files) && d.files.every((f) => typeof f === 'string' && f.trim()))) {
    ...
    // A stale path is how a hand-kept list rots silently: /diagnose routes on it
    // and simply never matches. Repo-relative only, so "exists" means IN THE TREE.
    const wr = typeof d.text === 'string' && d.text.includes('workflow_run');
...
console.log(`check-learnings: OK — ${lines.length} entries, all well-formed; every listed file exists (workflow_run entries: each listed workflow still contains the text). Completeness NOT checked: a watcher no entry names is still unrouted (#370)`);
