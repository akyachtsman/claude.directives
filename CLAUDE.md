...
node .github/scripts/check-links.js --internal   # offline: verifies against the working tree. Its OK line counts only what it could PARSE — the NOTE lists every reference it could not, for want of italic delimiters OR an unclosed `*` running past the length cap, and those are NOT verified. 21 are live here today
node .github/scripts/check-links-cases.js        # that checker's own guard — a reference it cannot parse is absent from both sides of "N/N resolve", so partial blindness reads as full coverage. Measured: a name wrapped across a line was invisible and 20 were live here (#363). The fix then grew its OWN version of the same defect TWICE — an over-long match dropped from the NOTE as well as the count, then an `*` opened and never closed matching neither producer — so the scan now enumerates reference SITES and subtracts the ones it parsed, rather than listing bad spellings. A site needs a single `*`, not `**`: eleven correct `→ **Bold**` sentences were reported as broken before that, and a NOTE full of false entries is read as little as no NOTE at all. Re-prove with CHECK_LINKS_BIN=<mutant> (a RELATIVE one now resolves against the caller, not the temp tree; before #365 it crashed every case and read as perfect discrimination)
