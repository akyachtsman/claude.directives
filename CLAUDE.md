...
python3 .github/scripts/check-learnings-derived.py   # an entry whose `files` list is DERIVABLE must equal the tree — adding a watcher cannot leave the entry correct-looking and the file unrouted. PyYAML, not a text scan: it WAS a scan for two rounds and was wrong both times (a commented-out snippet counted; four valid spellings read as no-match), and workflow-ref-guard.py's header already carried the verdict on that approach
python3 .github/scripts/check-learnings-derived-cases.py  # that guard's own guard — the live tree spells the trigger exactly ONE way, so every other form it claims to handle is unexercised, and it prints the same OK either way. Re-prove with CHECK_LEARNINGS_DERIVED_BIN=<mutant>
