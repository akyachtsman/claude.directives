...
node .github/scripts/check-links.js --internal   # offline: verifies against the working tree. Its OK line counts only the forms it PARSES — the italic `file.md` → *Name* / → *Name* — and SAYS SO on every run; a reference written any other way is absent from that fraction and is NOT verified (#366). The NOTE beneath it lists this parser's own DISCARDS (a name past the length cap), which is a closed set; it is not a list of what the parser failed to see, and #365 proved that list cannot be built by pattern-matching
node .github/scripts/check-links-cases.js        # that checker's own guard — a reference it cannot parse is absent from both sides of "N/N resolve", so partial blindness reads as full coverage. Measured: a name wrapped across a line was invisible and 20 were live here (#363). #365 then tried to REPORT everything unparseable and took three review rounds, each finding another spelling the enumeration missed (an unterminated `*`, a wrap before the arrow, a filename outside the character class) — so that set is open, the claim to have closed it was itself the fail-open, and the scan states its scope as a DISPOSITION instead. Do not add a case asserting some new malformed spelling is reported; widening belongs in #366. Re-prove with CHECK_LINKS_BIN=<mutant> (a RELATIVE one resolves against the caller, not the temp tree; left relative it crashed every case and read as perfect discrimination)
