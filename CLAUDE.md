...
python3 .github/scripts/check-action-siblings-cases.py  # that guard's own guard — everything in templates/actions/ ships, so the guard prints the same OK whether its rule works or does nothing. Real `git init` fixtures, because three cases turn on states no directory of loose files can express: a `git add -N` placeholder (in `ls-files`, absent from the tree), a directory SYMLINK (`os.walk` puts it in `dirnames` and never lists it), and an index mid-merge that `write-tree` cannot build at all. Each refusal has its accepting complement, so none can be bought by over-tightening. Re-prove with CHECK_ACTION_SIBLINGS_BIN=<mutant>
