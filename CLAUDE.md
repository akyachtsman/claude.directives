...
node .github/scripts/check-links.js --internal   # offline: verifies against the working tree. Its OK line counts only what it could PARSE — the NOTE lists every reference it could not, for want of italic delimiters OR an unclosed `*` running past the length cap, and those are NOT verified. 20 are live here today
node .github/scripts/check-links-cases.js        # that checker's own guard — a reference it cannot parse is absent from both sides of "N/N resolve", so partial blindness reads as full coverage. Measured: a name wrapped across a line was invisible and 20 were live here (#363). The fix then grew its OWN version of the same defect — an over-long match was dropped from the NOTE as well as the count — so the unparseable cases come in pairs, one per arm. Re-prove with CHECK_LINKS_BIN=<mutant> (a RELATIVE one now resolves against the caller, not the temp tree; before #365 it crashed every case and read as perfect discrimination)
