...
python3 .github/scripts/check-action-siblings.py   # every file under `templates/actions/*/` is TRACKED — present-and-untracked ships to nobody and looks identical to correct on the machine that wrote it (#325, #353). Reads NO shell: two designs that read text (matching install carriers, then deriving references from run bodies) produced defects across seven rounds, so the question changed to one with no syntax to miss. It does NOT check that carriers install these files, nor that a composite names a file that exists — both stated in its own passing verdict
python3 .github/scripts/check-action-siblings-cases.py  # that guard's own guard — everything in templates/actions/ is tracked, so the guard prints the same OK whether its rule works or does nothing. Real `git init` fixtures: one case turns on a `git add -N` placeholder, which is in `ls-files` and still does not ship. Re-prove with CHECK_ACTION_SIBLINGS_BIN=<mutant>
