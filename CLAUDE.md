...
node .github/scripts/check-links.js --internal   # offline: verifies against the working tree. Its OK line counts only what it could PARSE — the NOTE lists references skipped for want of italic delimiters, and those are NOT verified
node .github/scripts/check-links-cases.js        # that checker's own guard — a reference it cannot parse is absent from both sides of "N/N resolve", so partial blindness reads as full coverage. Measured: a name wrapped across a line was invisible and 20 were live here (#363). Re-prove with CHECK_LINKS_BIN=<mutant>
