...
python3 .github/scripts/check-action-siblings.py   # every file a composite runs by path (`$GITHUB_ACTION_PATH/…`) is TRACKED — an untracked one passes an existence check on the machine that wrote it and ships to nobody (#325, #353). It does NOT check that any carrier installs them: that half was removed in #354 after three rounds of defects, because "does this prose install this file" is not decidable from text and a guard wrong in the certifying direction is worse than none
python3 .github/scripts/check-action-siblings-cases.py  # that guard's own guard — this repo has exactly ONE such sibling, so the guard prints the same OK whether its rules work or do nothing at all. Real `git init` fixtures, because tracked-vs-present cannot be expressed with loose files. Re-prove with CHECK_ACTION_SIBLINGS_BIN=<mutant>
