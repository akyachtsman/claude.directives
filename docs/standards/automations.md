...
- **Unsubscribe before merging** via `unsubscribe_pr_activity` — past that point
  every delivered event is a notice about the merge you are performing. Before
  that, KEEP it: this is the only push signal for a review round, and dropping it
  disables `ci-notify.yml`'s wake (Automation 4c). A scheduled check-in is not a
  substitute — a timer cannot know whether anything changed
...
- Unsubscribe BEFORE merging, not after (`git.md` → *PR Lifecycle*). If the merge is then blocked, re-subscribe (the call is idempotent)
