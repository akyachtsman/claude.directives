...
- Triggers: `pull_request_review` AND `issue_comment`, both filtered to
  ...
  the two exits `git.md` → *PR Lifecycle* names, with the rationale that rule
  requires. Waiting on the monitor without doing either blocks the PR
  indefinitely.
...
- The `codex-flagged` label is **asymmetric**: a label still present blocks the merge
  ...
  removal is the last resort `git.md` → *PR Lifecycle* bounds to two exits. Apply `git.md` →
  *PR Lifecycle*'s source-review gate: match a Codex **review, comment, or inline
  ...
  clear it; take the label off by hand only on the two exits `git.md` → *PR
  Lifecycle* names
