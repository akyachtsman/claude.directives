...
- Do not merge on the `codex-flagged` label alone — it is written asynchronously, so its
  ...
  Codex review to HEAD by commit SHA (or a 👍 whose review request postdates the last
  push), read its inline comments, and treat unreadable reviews as uncleared
