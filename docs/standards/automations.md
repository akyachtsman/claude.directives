...
- **Unsubscribe before merging** via `unsubscribe_pr_activity` — past that point
  every delivered event is a notice about the merge you are performing. A repo
  may go further and drop subscriptions immediately (this one does — see its
...
- Unsubscribe BEFORE merging, not after (`git.md` → *PR Lifecycle*). If the merge is then blocked, re-subscribe (the call is idempotent) — **unless** the repo opts into immediate unsubscription, as Automation 5 describes; there, stay unsubscribed and use a scheduled check-in instead
