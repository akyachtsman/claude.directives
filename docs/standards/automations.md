...
- Do not merge on the `codex-flagged` label alone — it is written asynchronously, so its
  absence proves nothing. Apply `git.md` → *PR Lifecycle*'s source-review gate: match a
  Codex review — or its clean comment, which names the commit too — to HEAD by commit
  SHA, read the review's inline comments, and treat unreadable reviews as uncleared.
  Only a bare 👍 lacks a SHA; accept that alone only when the review request that
  triggered it postdates the last push
