...
- The `codex-flagged` label is **asymmetric**: a label still present blocks the merge
  ...
  removal is the last resort `git.md` → *PR Lifecycle* bounds by its *unreachable-review test*. Apply `git.md` → *PR Lifecycle*'s source-review gate: match a Codex **review, comment, or inline
  ...
  `pr-mechanics.md` → *The reaction ladder* carries the ladder. Its test is an ORDERING (push →
  ...
  clear it; take the label off by hand only where `git.md` → *PR Lifecycle*'s *unreachable-review test* admits it
