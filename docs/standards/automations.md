...
- The `codex-flagged` label is **asymmetric**: a label still present blocks the merge
  ...
  (Automation 3 above) — never on the 👍 an ordinary clean rerun leaves, which fires no
  trigger — so a label still present means concerns not yet re-reviewed, a reaction-only
