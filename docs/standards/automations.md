...
- The `codex-flagged` label is **asymmetric**: a label still present blocks the merge
  ...
  *PR Lifecycle*'s source-review gate: match a Codex **review, clean comment, or inline
  review-thread reply** — all three name the commit — to HEAD by commit SHA, confirm the
  Codex bot authored it, read the review's inline comments, and treat unreadable
  responses as uncleared. The inline-reply form clears the GATE but not the LABEL. **A bare 👍
