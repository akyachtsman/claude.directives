...
- The `codex-flagged` label is **asymmetric**: a label still present blocks the merge
  until triaged and removed with a rationale, but its absence proves nothing — it is
  written asynchronously, and it survives every later head once set. Apply `git.md` →
