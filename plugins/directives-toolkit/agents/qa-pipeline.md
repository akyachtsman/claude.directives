...
- Name `model` on every subagent you start (`global.md` → *Subagent Model Selection*):
  `sonnet` for test-verifier, the reviewers and pr-readiness-reviewer; the
  session's own model for ui-tester, whose loop produces the fixes
