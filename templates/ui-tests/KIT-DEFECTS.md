...
- **Check:**
  ```bash
  s2=$(mktemp); awk '/^test\(.S2:/,/^}\);/' tests/app.spec.js > "$s2"
  if [ ! -s "$s2" ]; then echo UNDECIDED
  elif grep -A4 -E "if \(.*(mechanism === 'none'|!s2Gated)" "$s2" | grep -q throw; then echo CLEAR
  else echo AFFECTED; fi; rm -f "$s2"
  ```
  `AFFECTED` = no `throw` within four lines of a condition on `mechanism === 'none'` or `!s2Gated` inside S2. Measured: `AFFECTED` on the kit at `fce69d9` and `ce2140a`; `CLEAR` on the kit since `1d57879`, on `ce2140a` plus the fix below, and on a `claude.trading`-style one-line `throw`; `UNDECIDED` on a file with no top-level S2.
