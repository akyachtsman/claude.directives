...
Entries are never deleted and ids are never reused. Each entry carries a `Revision:` number, starting at 1. A wrong entry is corrected IN PLACE: bump `Revision:` and add a `Corrected (rN):` line saying what changed. `/refresh-repo` always reads this file from the upstream head, so a correction takes effect on the next refresh, whereas a later entry could not stop the wrong fix from being applied first (#373). A project's recorded decline names the revision it judged (`KD-1 r1 declined …`); an entry whose `Revision:` is higher voids that decline and is re-checked. Revisions, not dates, because a same-day correction and decline cannot be ordered by date.
